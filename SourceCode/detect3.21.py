...
def detect_part(img):
    ...
    # cv2.imshow('111', img)
    # cv2.waitKey(0)
    corner_flag, x11, y11, temp_img, corners = select_corners(temp_img)
...
def detect_img(img):
    ...
    #
    # cv2.imshow("img1", img1)
    corner1_flag, dst1, corner1, corners1 = detect_part(img)
    flag = False
    if corner1_flag:
...
if __name__ == '__main__':
    ...
    with MTWindow(title="Metavision SDK Get Started", width=width, height=height,
        ...
        event_frame_gen = PeriodicFrameGenerationAlgorithm(sensor_width=width,sensor_height=height,accumulation_time_us=5000,fps=120.0,palette=ColorPalette.Dark)
    #
        def on_cd_frame_cb(ts,cd_frame):
            window.show_async(cd_frame)
